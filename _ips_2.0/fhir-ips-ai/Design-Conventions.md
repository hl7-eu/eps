# Design Conventions - International Patient Summary Implementation Guide v2.0.0
...
Having this clear is important for correctly understanding the published profiles. For example, the optional section of [Social History](./StructureDefinition-Composition-uv-ips-definitions.md#Composition.section:sectionSocialHistory.entry) has open slicing on the entry element allowing for the use of the [IPS Tobacco Use profile](./StructureDefinition-Observation-tobaccouse-uv-ips.md), the [IPS Alcohol Use profile](./StructureDefinition-Observation-alcoholuse-uv-ips.md), or any other Observation or DocumentReference. Therefore, while specific IPS profiles are described in this guide, other profiles may also be included as well.

### Narrative and Language Translation

#### Rationale for Narrative Requirements in IPS

When an [IPS Composition](./StructureDefinition-Composition-uv-ips.md) is generated, the resulting document may be viewed in many different ways (e.g. rendered by a viewer application, converted into a printable document, etc.). The IPS Composition includes a [requirement](./StructureDefinition-Composition-uv-ips-definitions.md#Composition.section.text) for each section to have human-readable narrative text. This includes the 16 clinical sections profiled in the [IPS Composition](./StructureDefinition-Composition-uv-ips.md) as well as any other section included in the patient summary. This aligns with the principles of [FHIR Documents](https://hl7.org/FHIR/documents.html) and has importance in the context of cross-border IPS exchange. Specifically:
...
* The functional requirement of supporting coded translations has been addressed in this guide by extending the [CodeableConcept datatype](./StructureDefinition-CodeableConcept-uv-ips.md) and [Coding datatype](./StructureDefinition-Coding-uv-ips.md). In addition, the use of [SNOMED as a primary IPS terminology](./General-Principles.md#structuring-terminology-choices) facilitates translation capabilities based on coded data. For other details about the support of narrative translations, please refer to [Multi-Language support in FHIR](https://hl7.org/fhir/R4/narrative.html#lang).
...
Additional perspective on the alignment of IPS to broader guidance around clinical documents in FHIR are included in the [Alignment to FHIR Clinical Documents](./About.md#alignment-to-fhir-clinical-documents) section.
...
**Figure 4: Example of Using textLink extension**

![](textLink.png)

> The extension registry that includes the[textLink](https://build.fhir.org/ig/HL7/fhir-extensions/StructureDefinition-textLink.html)extension shown above has not been officially published as of the current IPS publication. Implementations can proactively adhere to the resolution of JIRA ticket[FHIR-43316](https://jira.hl7.org/browse/FHIR-43316), and a reference to the published textLink extension will be incorporated into the next version of this IG.
